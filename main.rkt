...
(define-struct mybird (x y v-y)#:transparent)
(define-struct mypipe (gap-y x)#:transparent)
(define-struct mystate (bird pipes score) #:transparent)

(define (get-initial-state)
  (define bird (mybird bird-x bird-initial-y 0))
  (define pipe0 (mypipe (+ added-number (random random-threshold)) scene-width))
  (define state (mystate bird (list pipe0) 0))
  state)
...
(define (next-state-bird bird gravity)
  (match-let ([(mybird _ y v-y) bird])
  (struct-copy mybird bird [y (+ y v-y)][v-y (+ v-y gravity)])))
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy mybird bird [v-y (- 0 momentum)]))
...
(define (change current-state pressed-key)
    (cond [(key=? pressed-key " ") (struct-copy mystate current-state [bird (next-state-bird-onspace (get-bird current-state) initial-momentum)])]
          [else current-state]))
...
(define (move-pipes pipes scroll-speed)
  (map (lambda (pipe)   (match-let ([(mypipe _ x) pipe])
  (struct-copy mypipe pipe [x (- x scroll-speed)]))) pipes))
;TODO 12
...
(define (clean-pipes pipes)
  (filter (lambda (pipe) (< 0 (+ pipe-width (get-pipe-x pipe)))) pipes))
...
(define (add-more-pipes pipes)
  (if (> no-pipes (length pipes))
      (append pipes (list (make-mypipe (+ added-number (random random-threshold)) (+ (+ pipe-width pipe-gap) (get-pipe-x (last pipes))))))
      pipes)
...
(define (check-pipe-collisions bird pipes)
  (foldr (λ(x y) (or x y)) #f (map (λ(pipe) (if (or
  (check-collision-rectangles (make-posn (mybird-x bird) (get-bird-y bird))
                              (make-posn (+ (mybird-x bird) bird-width) (+ (get-bird-y bird) bird-height))
                              (make-posn (mypipe-x pipe) (+ (mypipe-gap-y pipe) pipe-self-gap))
                              (make-posn (+ pipe-width (mypipe-x pipe)) scene-height))
  (check-collision-rectangles (make-posn (mybird-x bird) (get-bird-y bird))
                              (make-posn (+ (mybird-x bird) bird-width) (+ (get-bird-y bird) bird-height))
                              (make-posn (mypipe-x pipe) 0)
                              (make-posn (+ pipe-width (mypipe-x pipe)) (mypipe-gap-y pipe)))
    )
                    #t
                    #f)) pipes))
  )
...
(define (next-state state)
    (match-let ([(mystate bird pipes score) state])
    (struct-copy mystate state [bird (next-state-bird bird initial-gravity)] [pipes (next-state-pipes pipes initial-scroll-speed)] [score (+ 0.1 score)]))
  )
...
(define (draw-frame state)
    (place-image bird-image (+ (mybird-x (mystate-bird state)) (quotient bird-width 2)) (+ (mybird-y (mystate-bird state)) (quotient bird-height 2)) 
                (place-image ground-image (quotient scene-width 2) (- scene-height (quotient ground-height 2))
                            (place-image (score-to-image (mystate-score state)) text-x text-y (place-pipes (mystate-pipes state) initial-scene)))))

; Folosind `place-image/place-images` va poziționa pipe-urile pe scenă.
(define (place-pipes pipes scene)
  (if (null? pipes) scene
      (place-pipes (cdr pipes) (place-image pipe-image (+ (get-pipe-x (car pipes)) (quotient pipe-width 2)) (- (mypipe-gap-y (car pipes)) (quotient pipe-height 2))
                                            (place-image pipe-image (+ (get-pipe-x (car pipes)) (quotient pipe-width 2))
                                                         (+ (mypipe-gap-y (car pipes)) (+ pipe-self-gap (quotient pipe-height 2) )) scene)))))
